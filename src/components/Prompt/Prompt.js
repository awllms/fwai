...
const fetchUrl = "https://fwaiapi-awllms-projects.vercel.app";
// const fetchUrl = "http://localhost:3001";

export const Prompt = () => {
  const [textAreaValue, setTextAreaValue] = useState("");
  const [selectBoxValue, setSelectBoxValue] = useState("gpt-3.5-turbo");
  ...
  useEffect(() => {
    if (
      ...
      JSON.parse(localStorage.getItem("responses")).length
    ) {
  ...
  useEffect(() => {
    if (
      responses.indexOf(nextPrompt) === -1 &&
  ...
  return (
    <section className="Prompt">
      ...
      <Responses
        isloading={isLoading}
        responses={responses}
        onResponsesClearButtonClick={onResponsesClearButtonClick}
      />
    </section>
  );
};
